...
import syslog #kirjoitetaan virheet syslogiin
...
while True:
    try:
        ...
        if lampo22 is not None:
         ...
         #kirjoitetaan syslogiin virhe
	 syslog.syslog (time.strftime("%H:%M:%S ") + "Lampotilatietoa ei saatavilla")
	...
	if kosteus22 is not None:
         ...
         #kirjoitetaan syslogiin virhe
	 syslog.syslog (time.strftime("%H:%M:%S ") + "Kosteustietoa ei saatavilla")
